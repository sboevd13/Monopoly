...
fun PlayerRow(
    ...
    // Состояние для хранения списка домов
    var houses by remember { mutableStateOf<List<HouseListDTO>?>(null) }
    // Состояние для отслеживания, раскрыт ли список
    var isExpanded by remember { mutableStateOf(false) }
    // Состояние для отслеживания загрузки
    val isLoading = houses == null

    // Загружаем дома только один раз, когда список пытаются раскрыть впервые
    LaunchedEffect(isExpanded) {
        // Если список раскрыт и дома еще не загружены
        if (isExpanded && houses == null) {
            houses = houseViewModel.loadPlayerHouses(player.id)
        }
    }

    // Анимация для плавного раскрытия/сворачивания
    Column(modifier = Modifier.animateContentSize()) {
        // Верхняя строка с основной информацией об игроке, она же кнопка для раскрытия
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .clickable { isExpanded = !isExpanded } // Переключаем состояние по клику
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = "Капитал: ${player.capital}$",
                    fontSize = 14.sp,
                    color = Color.Gray
                )
            }

            // Иконка-стрелка, которая вращается в зависимости от состояния
        ...
        // Если список раскрыт, показываем контент
        if (isExpanded) {
            // Вложенный контент с отступом
            Box(modifier = Modifier.padding(start = 16.dp, top = 8.dp, bottom = 8.dp)) {
                when {
                    ...
                    // Если дома загружены, но их нет
                    houses?.isEmpty() == true -> {
                        Text(
                            text = "У этого игрока нет домов.",
                            color = Color.Gray,
                            fontSize = 14.sp
                        )
                    }
                    // Если дома есть, показываем их списком
                    else -> {
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            houses?.forEachIndexed { index, house ->
                                HouseInfoCard(house = house, houseNumber = index + 1)
                            }
                        }
                    }
                }
...
/**
 * Карточка для отображения детальной информации об одном доме.
 */
@Composable
fun HouseInfoCard(house: HouseListDTO, houseNumber: Int) {
    Card(
        modifier = Modifier.fillMaxWidth(),
        // 1. Правильный способ задать elevation в M3
        elevation = CardDefaults.cardElevation(defaultElevation = 2.dp),
        // 2. Правильный способ задать цвета в M3
        colors = CardDefaults.cardColors(
            containerColor = MaterialTheme.colorScheme.surfaceVariant // Используем цвета из colorScheme
        )
    ) {
        Column(modifier = Modifier.padding(12.dp)) {
            Text(
                text = "Дом №$houseNumber",
                fontWeight = FontWeight.Bold,
                fontSize = 16.sp,
                // 3. Используем MaterialTheme.colorScheme вместо MaterialTheme.colors
                color = MaterialTheme.colorScheme.primary
            )
            Spacer(modifier = Modifier.height(8.dp))

            if(house.construction_time > 0){
                InfoRow("Стоимость квартиры:", "${house.stoimost_kvartyry}$")
                InfoRow("Количество квартир:", "${house.kolichestvo_kvartir}")
                InfoRow("Время строительства:", "${house.construction_time} мес.")
            }else if (house.kolichestvo_kvartir > 0) {
                InfoRow("Стоимость квартиры:", "${house.stoimost_kvartyry}$")
                InfoRow("Количество квартир:", "${house.kolichestvo_kvartir}")
            }else{
                Text("Все квартиры проданы")
            }
        }
    }
}

// Вспомогательная функция InfoRow остается без изменений, но убедитесь,
// что Text в ней использует импорт из androidx.compose.material3.Text
@Composable
            ...
            Surface(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Row(modifier = Modifier.fillMaxWidth()) {
                        ...
                        TextButton(
                            onClick = {
                                if (houseState.ezemes_platez > playerState.capital){
                                    Toast.makeText(context, "У вас недостаточно денег", Toast.LENGTH_LONG).show()
                                }else{
